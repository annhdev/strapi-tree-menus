...
const MenuDataProvider = ({ children, value, onChange, name, schema, disabled, error }: MenuDataProviderProps) => {
  ...
  const flattenedItems = useMemo(() => {
    ...
    const collapsedItems = flattenedTree.reduce<UniqueIdentifier[]>(
      (acc, { children, collapsed, errors, id }) =>
        collapsed && children.length ? [...acc, errors, id] : acc,
      [],
    );
  ...
  const handleValidate = useCallback(async (items: TreeItems) => {
    try {
  ...
  return (
    <MenuDataContext.Provider value={{ errors, setErrors, flattenedItems, activeId, setActiveId, items, setItems, activeItem, setActiveItem, onChange: handleOnChange, value, name, schema, disabled, error, validate: handleValidate }}>
      {children}
