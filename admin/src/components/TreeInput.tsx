...
const TreeInput = React.forwardRef<HTMLButtonElement, TreeInputProps>(({ hint, disabled = false, labelAction, label, name, required = false, onChange, value = [], error, ...props }, forwardedRef) => {

  const { attribute: { options: { schemas } } }: FieldOptions = props;
  ...
  return (
    <>
      <MenuDataProvider name={name} value={value} onChange={onChange} schema={_schemas} disabled={disabled} error={error}>
